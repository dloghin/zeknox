...
fn bench_gpu_ntt_batch(c: &mut Criterion) {
    ...
    for log_ntt_size in log_ntt_sizes {
        ...
        group
            .sample_size(10)
            .bench_function(&format!("CPU FFT with n of size 2^{}", log_ntt_size), |b| {
                b.iter(|| cpu_ntt(domain_size, &cpu_buffer, batches))
            });
    }
}

pub fn cpu_ntt(domain_size: usize, cpu_buffer: &Vec<u64>, batches: usize) -> Vec<u64> {
